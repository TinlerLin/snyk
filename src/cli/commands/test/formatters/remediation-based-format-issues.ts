...
function processUpgrades(
  ...
  for (const dep of deps) {
    ...
    const vulnIds =
      ...
      const upgradeText = `\n  Upgrade ${chalk.bold.whiteBright(dep.split('@')[0])} from ${chalk.bold.whiteBright(dep.split('@')[1])} to ${chalk.bold.whiteBright(upgradeDepTo.split('@')[1])} \n`;
    sink.push(
...
export function formatIssue(
  ...
  return (
    severityColor.colorFunc(`\n  ✗ [${titleCaseText(severity,)}${originalSeverityStr}] ${chalk.bold(title)}${newBadge} \n`) +
    reachabilityText +
